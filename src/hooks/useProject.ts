...
// fallback (see docs/DECISIONS.md). `generated_sections` is then carried
// forward from whatever's already in the TanStack Query cache (populated by
// useProject, which does fetch/fall back on that column) rather than
// re-read, since nothing in a facts or conflict write ever changes it.
// Whether any of it is fixture-sourced lives on the content itself
// (`GeneratedSectionContent.source`, see hasFixtureGeneratedSections) —
// carrying the object forward as-is is enough, there's no separate flag to
// keep in sync.
...
async function fetchProjectRow(projectId: string): Promise<ProjectRow> {
  ...
  if (error) {
    if (isDemoMode() && isMissingSchemaError(error)) {
      ...
      return {
        ...(fallback as Omit<ProjectRow, 'generated_sections'>),
