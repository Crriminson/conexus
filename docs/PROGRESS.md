...
## Decoupled the "Demo data" banner from persistence — content provenance instead

Context confirmed by a human via `supabase secrets list`: the server `DEMO_MODE` secret is genuinely off, not just undocumented. That makes `docs/FIXTURE_INVENTORY.md` §4.1's dormant risk a precondition to close before anyone turns it on, not a hypothetical — this task closes it. No live invocation of `generate-section` (would spend Gemini quota) — verified entirely with mocked `callLLM` responses via the existing `createMockProvider` pattern, same as every other DEMO_MODE test in this codebase.

**Root cause, confirmed from source before touching anything (per the task's own instruction):** the "Demo data" banner (`DocumentValidationPanel.tsx`) read `ProjectRow.generatedSectionsIsDemo`, a boolean set at 3 separate call sites — `useProject.ts`'s two `fetchProjectRow` branches (real read vs. schema-missing fixture fallback) and `useGenerateSections.ts`'s `onSuccess` (keyed off `result.persisted`, i.e. whether the DB write landed). That third site is where content provenance and persistence outcome got conflated: `generate-section/index.ts` can independently succeed or fail at the Gemini call *and* independently succeed or fail at persisting, so all four combinations are possible, but the banner only ever asked "did the write land," never "is this text actually fake."

**The fix: `GeneratedSectionContent` now carries its own `source: 'real' | 'fixture'`, set once, at the only point each fallback actually knows which path it took — never re-derived later.**
- `resolveGeneratedSections()` (`supabase/functions/_shared/generatedSections/`) takes a new required 4th `source` parameter and stamps it on every section it resolves — required, not defaulted, so every call site has to make an explicit choice rather than the tag being inferrable or forgettable.
- `generate-section/index.ts` tracks a local `contentSource`, `'real'` by default, flipped to `'fixture'` in the exact `catch` block where it already falls back to `buildDemoGeneratedSectionsResponse` — the only change to when anything triggers is none; this is purely bookkeeping alongside the existing branch.
- `fixtureGeneratedSections()` (`src/lib/templates/fixtures.ts`, `useProject`'s own schema-missing fallback) stamps `source: 'fixture'` on its 3 canned sections directly.
- New pure function `hasFixtureGeneratedSections(sections)` (`supabase/functions/_shared/generatedSections/hasFixtureGeneratedSections.ts`, re-exported through both the shared and client `index.ts` files like every sibling in that module) — `true` when any section present has fixture-sourced content. This *is* the banner's trigger condition now: `DocumentValidationPanel.tsx` calls it directly on `project.generated_sections` instead of reading any stored flag.
- `ProjectRow.generatedSectionsIsDemo` is deleted outright — `useProject.ts`, `useGenerateSections.ts`, `DocumentValidationPanel.tsx` all updated. Nothing else referenced it (grepped to confirm). Structurally, the banner can no longer disagree with the content, because it's reading the content's own tag rather than a parallel flag that had to be kept in sync by hand.

**What this actually changes, in terms of the 4 combinations:** real content that fails to persist no longer gets mislabeled "Demo data" (the inverse-accuracy bug FIXTURE_INVENTORY.md §4.1 flagged as secondary) — the banner now correctly stays off. Fixture content that *does* persist (the scenario that activates once the server secret + migration both land) now correctly triggers the banner — previously it wouldn't have, since `persisted: true` used to skip setting the flag entirely. Both fixture-content cases and both real-content cases behave identically regardless of persist outcome, which is the whole point of decoupling them.

**Tests:** 6 new, all in `src/lib/generatedSections/generatedSections.test.ts` (169 total, up from 163) — a direct test that `resolveGeneratedSections` stamps whichever `source` it's given, `source` assertions added to the existing mock-provider and `buildDemoGeneratedSectionsResponse` tests (representing the real and fixture paths respectively), and a new `describe('hasFixtureGeneratedSections — the banner trigger, decoupled from persistence')` block with one test per named combination from the task (real+persisted, real+failed, fixture+persisted, fixture+failed, plus an empty-object case) — two of those four collapse to literally the same assertion, which the test comments call out explicitly as the fix itself: persistence stops being a parameter the banner's logic even takes.

**`tsc -b`/`vite build`/`vitest run` (169/169)/`oxlint` all clean** (same 2 pre-existing unrelated warnings as always). Not done, per the task's explicit boundary: the server `DEMO_MODE` secret was not set, and nothing about *when* either fallback triggers was touched — only how provenance is tracked and how the banner reads it.

**Files:** `supabase/functions/_shared/generatedSections/{types,resolveGeneratedSections,hasFixtureGeneratedSections(new),index}.ts`, `supabase/functions/generate-section/index.ts`, `src/lib/generatedSections/index.ts`, `src/lib/templates/{fixtures,templates.test}.ts`, `src/lib/generatedSections/generatedSections.test.ts`, `src/hooks/{useProject,useGenerateSections}.ts`, `src/components/document/DocumentValidationPanel.tsx`.

**Outstanding:** none from this task's scope. Setting the server `DEMO_MODE` secret and applying the `generated_sections` migration are both manual, out-of-band steps for whoever has real project access — this task was the precondition, not the switch-flip.
