...
## Full-app-completion phase, Step 5 (continued) — Task 12 live check switched to the real ANP dataset

`docs/STATE.md` already called out that the ANP dataset (once its conflicts are resolved) is a better basis for the Task 12 live check than `scripts/seed-task12-check.sh`'s synthetic "Meridian Textiles Limited" seed — real extracted+confirmed facts, and a real Storage-backed document behind the citations, versus hand-written fixture data with a fake document row that always 404'd on citation clicks. Acted on that decision now rather than leaving it for whoever runs the check:

- `scripts/seed-task12-check.sh` no longer writes anything. Rewritten into a read-only readiness gate: checks zero pending `FactConflict`s on the live project and that `company.cin` still matches the expected ANP value (i.e. the dataset hasn't been torn down or overwritten). Verified against live data both ways — currently fails correctly (3 conflicts still pending, see the entry above) and the CIN-match branch was confirmed to pass in isolation.
- `docs/TASK12_LIVE_CHECK.md` rewritten throughout: intro explains the switch, pre-flight gains a 3rd item (conflicts resolved via Facts Review — itself gated on the same migration, per the entry above), step 1 is now "verify readiness" instead of "seed", the browser-check table's citation-click row (2.6) now expects a real signed URL to actually work instead of the previous "expected 404", and section 4 (teardown) now explicitly warns against running `scripts/teardown-task12-check.sh` against this dataset — it would wipe the real ANP facts, not disposable seed data — with an optional generated_sections-only reset snippet instead.
- `scripts/teardown-task12-check.sh` itself was deliberately **not** touched (out of scope for this pass) — it's now stale for this flow (targets a `SEED_DOCUMENT_ID` that doesn't exist in the ANP dataset) but is left as-is since `docs/TASK12_LIVE_CHECK.md` no longer points at it. Worth a follow-up cleanup pass, not urgent since nothing currently calls it.

**Files:** `docs/TASK12_LIVE_CHECK.md`, `scripts/seed-task12-check.sh`, `docs/STATE.md`, `docs/PROGRESS.md`.
